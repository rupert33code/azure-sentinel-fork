...
function AttemptAzLogin($clientId, $tenantId, $cloudEnv) {
    $maxLoginRetries = 3
    $delayInSeconds = 30
    $retryCount = 1
    $stopTrying = $false
    do {
        try {
            ...
            Connect-AzAccount -Tenant $tenantId -Identity -AccountId $clientId -Environment $cloudEnv | out-null;
...
function ConnectAzCloud {
    ...
    AttemptAzLogin $clientId $tenantId $CloudEnv
